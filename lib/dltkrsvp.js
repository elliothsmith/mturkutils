...
(function (dltk /*, window -- commented as unused */) {
    ...
    dltk.RSVPModule = function RSVPModule(_$, optdct) {
        ...
        this._onBeginExpDefault = function _onBeginExpDefault() {
            if (o.responseKeys != null) {
                // Register key responses
                function checkKeyPressed(e){
                    if([32, 37, 38, 39, 40].indexOf(e.keyCode) > -1) { // Prevents window scrolling if using arrow keys.
                                e.preventDefault();}

                    if (testing_period) { // If on response phase
                        idx = o.responseKeys.indexOf(e.keyCode);
                        if (idx != -1) that._clickedTestBtn(idx); // if pressed key is in e.responseKeys, call ._clickedTestBtn(idx)
                    }
                }
