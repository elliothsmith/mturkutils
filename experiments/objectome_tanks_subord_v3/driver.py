...
def get_meta(selected_basic_objs=SELECTED_BASIC_OBJS):
    """Mix the objectome 64 basic-level set and the tank subordinate
    level set"""
    ...
    cnames = list(meta_tanks.dtype.names)
    ...
    meta = tb.tabarray(
        columns=[np.concatenate([meta_basic[e][si], meta_tanks[e]])
                 for e in cnames],
        names=cnames)
    assert len(meta) == 30 * 1000 * 2
    assert len(np.unique(meta['obj'])) == 60   # 30 non-tanks + 30 tanks
    return meta, meta_basic, meta_tanks
...
def get_url_labeled_resp_img(obj):
    return get_urlbase(obj).replace('_tanks_subord_v3/', '_tanks_subord/') \
        + 'label_imgs/' + obj + '_label.png'


def get_exp(sandbox=True, selected_basic_objs=SELECTED_BASIC_OBJS,
            debug=False):
    meta, _, meta_tanks = get_meta()
    tanks = np.unique(meta_tanks['obj'])
    combs = [(o1, o2) for o1 in selected_basic_objs for o2 in tanks] + \
            [e for e in itertools.combinations(tanks, 2)]
    ...
    exp = MatchToSampleFromDLDataExperiment(
            htmlsrc='web/tanks_subord.html',
            htmldst='tanks_subord_n%05d.html',
            sandbox=sandbox,
            ...
            comment="objectome_tanks_subord_v3.  30 tanks and 30 non-tanks",  # noqa
            collection_name='objectome_tanks_subord_v3',
            max_assignments=1,
            bucket_name='objectome_tanks_subord_v3',
