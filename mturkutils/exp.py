...
class Experiment(base.Experiment):
    ...
    def prepHTMLs(self):
        try:
            ...
            pass
        super(Experiment, self).prepHTMLs()
    ...
    def check_if_hits_are_completed(self):
        ...
        """
        results = base.download_results(self.get_hitids(), sandbox=self.sandbox)
    ...
    def get_workers(self):
        ...
        for subjid, hitid in enumerate(hitids):
            subj_data = self.getHITdata(hitid, verbose=False, full=False)
            if len(subj_data) == 1: workers.append(subj_data[0]['WorkerID'])
            ...
            if len(subj_data) == 0:  # no data yet
                ...
                for subj in subj_data:
                    ...
                    for k, (img, resp, rt) in enumerate(data):
                        imgid = base.getidfromURL(img[0])
                        assert imgid == self.exp_plan['id'][idx]
                        # respid = base.getidfromURL(resp)
